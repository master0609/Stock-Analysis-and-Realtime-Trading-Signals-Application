...
def analyze_stock(ticker, start_date, end_date, lookback_period=60):
    ...
    try:
        ...
        # Identify buy/sell signals based on RSI and EMA crossover - UPDATED
        signals = np.zeros_like(prices)
        
        for i in range(1, len(prices)):
            # Buy signal: RSI is below 40 (relaxed from 30) OR price crosses above 20 EMA
            if (rsi[i] < 40) or (prices[i] > ema_20[i] and prices[i-1] <= ema_20[i-1]):
                signals[i] = 1
                
            # Sell signal: RSI is above 60 (relaxed from 70) OR price crosses below 20 EMA
        ...
        print(f"Total signals: {signal_count} (Buy: {buy_count}, Sell: {sell_count})")
        print(f"RSI range: Min {np.min(rsi):.2f}, Max {np.max(rsi):.2f}")
        print(f"Next day prediction: ${next_day_price:.2f} ({'+' if price_change >= 0 else ''}{price_change:.2f}%)")
